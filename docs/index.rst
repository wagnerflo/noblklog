...
handlers it provides can be used with and application built on
...
If you build (or use) a more sophisticated application it is quite
propable that at one point you might provide configurable logging using
:func:`~logging.config.fileConfig` or :func:`~logging.config.dictConfig`
something like this.
...
See the `Library API <api>`_ part of the documentation.
...
logging that message queue will eat your memory.

In practice for normal sized software components with run of the mill
logging behaviour this is not a problem.
